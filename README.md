# RAG 학습 시스템 - 프론트엔드 개발팀의 LLM 이해 프로젝트

## 프로젝트 개요

이 프로젝트는 고피자 프론트엔드 팀이 개발한 RAG(Retrieval Augmented Generation) 시스템의 테스트용 토이 프로젝트입니다. 단순한 LLM API 호출을 넘어서, 벡터 데이터베이스, 임베딩, 컨텍스트 검색 등 RAG 아키텍처의 핵심 요소들을 이해하고 구현하기 위해 개발되었습니다.

프론트엔드 개발자로서 최신 AI 기술을 활용한 애플리케이션 개발 역량을 강화하고, 특히 문서 기반 질의응답 시스템의 내부 작동 원리를 실제 코드로 구현하여 이해하는 것을 목표로 합니다.
